...
import { Dialog, DialogContent, Typography, DialogActions, TextField, Button, FormControlLabel, Checkbox } from '@mui/material';
...
const EditDialog = (props: EditDialogProps) => {
    ...
    return (
        <Dialog open={true} onClose={onClose}>
            <DialogContent>
                <Typography gutterBottom variant="subtitle1" className='ticket-date'>{format(new Date(ticket.gift_date), 'yyyy-MM-dd E')}</Typography>
                ...
                {ticket.status === 'draft' && (
                    <>
                        ...
                        <FormControlLabel label="下書きを確定する" control={<Checkbox checked={willFinalize} onChange={event => setWillFinalize(event.target.checked)} />} />
                    </>
                )}
            </DialogContent>
            <DialogActions>
                <Button variant="contained" onClick={updateTicket}>修正する</Button>
                <Button variant="outlined" onClick={onClose} sx={{color: 'primary.dark'}}>キャンセル</Button>
            </DialogActions>
        </Dialog>
    );
};
